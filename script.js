(function(){
  ...
  /* ===== Shared background persistence on <html> ===== */
  (function(){
    const BG='plannerBackground', MODE='plannerBgMode', POSX='plannerBgX', POSY='plannerBgY';
    const html=document.documentElement, menu=$('#plannerMenu');
    ...
    function applyBg(){
      try{
        const saved=localStorage.getItem(BG); if(saved) html.style.backgroundImage='url('+saved+')';
    ...
    bgInput?.addEventListener('change', e=>{ const f=e.target.files[0]; if(!f) return; const r=new FileReader();
      r.onload=ev=>{ const img=ev.target.result; document.documentElement.style.backgroundImage='url('+img+')'; localStorage.setItem(BG,img); };
      r.readAsDataURL(f); bgInput.value=''; });
  ...
  function render(){
    ...
    state.forEach((d,i)=>{
      ...
      card.innerHTML=`
        <div class="toprow">
          <div class="left">
            <span class="date-label" role="button" title="Clic per modificare data">${fdate(d.dateISO)}</span>
      ...
      // Attachments (single paperclip per day) → insert into current editor at caret
      $('.attach', card).addEventListener('click', ()=>{
        ...
        input.onchange = async ()=>{
          ...
          let stored=true;
          try{ await window.AttachmentsDB.put(dbKey, f); }catch(e){ stored=false; }
          const type=f.type||''; const isImg = type.startsWith('image/');
          if(isImg){
            const img=document.createElement('img');
            img.className='inline-thumb'; img.alt=f.name;
            if(stored){ img.setAttribute('data-dbkey', dbKey); img.setAttribute('data-type', type); }
            else { const url=URL.createObjectURL(f); img.src=url; img.onclick=()=>window.InlineViewer?.open(url,type); }
            insertNodeInCurrentLi(target, img);
          }else{
            const a=document.createElement('a');
            a.className='inline-file'; a.href='#'; a.textContent=f.name;
            if(stored){ a.setAttribute('data-dbkey', dbKey); a.setAttribute('data-type', type||'application/pdf'); }
            else { const url=URL.createObjectURL(f); a.onclick=(e)=>{ e.preventDefault(); window.InlineViewer?.open(url,type||'application/pdf'); }; }
            insertNodeInCurrentLi(target, a);
          }
          hydrateInlineFiles(target);
          persist();
          input.value='';
        };
        input.click();
    ...
    for(const el of nodes){
      ...
      try{
        ...
        if(el.tagName==='IMG'){
          ...
          el.onclick = (e)=>{ e.preventDefault(); if(window.InlineViewer){ window.InlineViewer.open(url, type||'application/pdf'); } else { window.open(url,'_blank'); } };
        }
